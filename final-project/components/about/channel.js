import styles from '../../styles/about/channel.module.css'
function Channel() {
    return (
    <div id={styles.channel}>
        <div id={styles.channel1}>
            <div id={styles.logo}>
                    <a href="https://www.toutiao.com/">
                        <img src="data:image/png;base64,"/>
                    </a>
            </div>
            <div id={styles.channels}>
                <a href="https://mp.toutiao.com/profile_v4_public/public/protocol/complains/" target="_blank">
                    关于头条
                </a>
            </div>
            <div>
                <a href="https://jobs.bytedance.com/" target="_blank">
                    加入头条
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/report/" target="_self">
                    媒体报道
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/media_partners/" target="_self">
                    媒体合作
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/cooperation/" target="_self">
                    产品合作
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/media_cooperation/" target="_self">
                    合作说明
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/contact/" target="_self">
                    联系我们
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/user_agreement/" target="_self">
                    用户协议
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/complain/" target="_self">
                    投诉指引
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/corrupt_report/" target="_self">
                    廉洁举报
                </a>
            </div>
            <div>
                <a href="https://www.toutiao.com/friend_link/" target="_self">
                    友情链接
                </a>
            </div>
        </div>
    </div>
    )
}

export default Channel